...
router.post('/seed', async (req, res) => {
  try {
    ...
    res.status(201).json({ 
      status: 'success', 
      message: `${insertedProducts.length} products seeded successfully.` 
    });
  } catch (err) {
...
router.get('/filter', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: `Filter failed: ${err.message}` });
  }
});
...
router.get('/variants/:color', async (req, res) => {
  ...
  try {
    ...
    if (!variants.length) return res.status(404).json({ message: `No variants found for color "${color}".` });
